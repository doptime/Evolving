...
var ApiChatGptAPI, _ = api.Api(func(param *InChatGPT) (Answer *OutChatGPT, err error) {
	...
	for i := 0; i < int(param.maxPackN); i++ {
		if i == 0 {
...
func LoadCache(req *InChatGPT, LastMessage string) (Answer *OutChatGPT, err error) {
	if req.Model == "" {
		return nil, errors.New("LoadCache:Model is empty")
	}
	LastMessage, err = GptCacheGet(req.Model, req.Prompt)
