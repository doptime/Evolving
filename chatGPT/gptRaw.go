...
type InChatGPT struct {
	...
	Terminated         func(answer string) (result []string, done bool)
}
...
func GptChatSession(req *InChatGPT) func(Prompt string) (Answer *OutChatGPT, err error) {
	var (
	...
	defer func() {
		if ConversationID != "" && req.RemoveConversation {
			DeleteUrl := fmt.Sprintf(req.BaseUrl+"/%s", ConversationID)
			FakeAIDel(DeleteUrl, req.AccessToken)
		}
	}()

	ApiGptChatOnce := func(Prompt string) (Answer *OutChatGPT, err error) {
		var (
			message_id, LastMessage, answer string
			lines, lines1                   []string
		)
		if Answer, err = LoadCache(req, LastMessage, &ConversationID, &MsgID, &ModelSlug); err == nil {
			return Answer, nil
		}
		Answer = &OutChatGPT{}
		...
		postData := &FakeAIChatData{Action: "next", Messages: []*FakeAIChatMessage{messge}, Model: req.Model, Parent_message_id: MsgID,
			Conversation_id: ConversationID, HistoryAndTrainingDisabled: false}
		...
		ReportError(req.AccessToken, req.Model, err)
		return Answer, err
	}
	return ApiGptChatOnce

}
func GptLongDialogues(req *InChatGPT, Prompt string) (Answer *OutChatGPT, err error) {

	var (
		LastMessage                      string
		gptChat                          = GptChatSession(req)
		prompt                           = Prompt
		ConversationID, MsgID, ModelSlug string
	)
	if Answer, err = LoadCache(req, prompt, &ConversationID, &MsgID, &ModelSlug); err == nil {
		return Answer, nil
	}
	for i := 0; i < int(req.maxDialogueN); i++ {
		...
		if Answer, err = gptChat(prompt); err != nil {
			return nil, err
		}
		...
		if _, done := req.Terminated(LastMessage); done {
			break
		}
	}
